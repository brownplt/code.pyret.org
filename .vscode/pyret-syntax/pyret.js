CodeMirror.defineMode("pyret", function(config, parserConfig) {
  ...
  function tokenBase(stream, state) {
    ...
    if ((match = stream.match(pyret_double_punctuation, true)) ||
      ...
      if (p === "=>") {
        ...
        state.expectingCaseParams = false;
      }
      if (p === "::" || p === "->") {
      ...
      if (p === "(") {
        ...
        } else if (state.expectingCaseParams) {
          state.expectingCaseParams = false;
          state.inParamList = true;
          state.paramParenDepth = 1;
          if (!state.paramScopes) state.paramScopes = [];
        ...
        const currentBlock = (state.blockStack && state.blockStack.length > 0)
          ? state.blockStack[state.blockStack.length - 1]
          : null;
        if (currentBlock === "cases") {
          if (!state.paramScopes) state.paramScopes = [];
          if (state.paramScopes.length > 0 && state.paramScopes[state.paramScopes.length - 1].isBranch) {
            state.paramScopes.pop();
          }
          state.expectingCaseVariant = true;
          state.inParamList = false;
        } else {
          state.inParamList = false;
        }
    ...
    if ((match = stream.match(pyret_keywords, true))) {
      const kw = match[0];
      if (!state.blockStack) state.blockStack = [];
      if (kw === "data") {
        state.dataNoPipeColon = true;
        state.blockStack.push("data");
      } else if (kw === "fun" || kw === "lam" || kw === "method") {
        state.expectingFunParams = true;
        state.blockStack.push("fun");
      } else if (kw === "cases") {
        state.expectingFunParams = false;
        state.blockStack.push("cases");
      } else if (kw === "ask") {
        state.expectingFunParams = false;
        state.blockStack.push("ask");
      } else if (kw === "if" || kw === "when" || kw === "for" || kw === "check" || kw === "examples" || kw === "block" || kw === "table" || kw === "reactor" || kw === "try" || kw === "shared") {
        state.blockStack.push(kw);
      } else if (kw === "end") {
        state.inParamList = false;
        state.expectingFunParams = false;
        state.expectingCaseVariant = false;
        state.expectingCaseParams = false;
        if (state.blockStack.length > 0) {
          ...
          } else if (popped === "cases") {
            if (state.paramScopes && state.paramScopes.length > 0 && state.paramScopes[state.paramScopes.length - 1].isBranch) {
              state.paramScopes.pop();
            }
          }
        } else {
    ...
    if ((match = stream.match(pyret_keywords_colon, true))) {
      if (stream.peek() === ":")
        ...
        return ret(state, 'name', match[0], 'constant');
    }
    // Level 2
    if ((match = stream.match(pyret_ident_regex))) {
      const name = match[0];
      const isPascalCase = /^[A-Z][a-zA-Z0-9_-]*$/.test(name) && /[a-z]/.test(name);
      ...
      if (state.expectingCaseVariant) {
        state.expectingCaseVariant = false;
        if (stream.match(/\s*\(/, false)) {
          state.expectingCaseParams = true;
        }
        return ret(state, 'name', name, 'type');
      }
      ...
      if (stream.match(/\s*\(/, false))
        return ret(state, 'name', name, 'function-name');
      return ret(state, 'name', name, 'constant');
    }
    if (stream.eat("-"))
      return ret(state, '-', '-', 'builtin');
    stream.next();
  ...
  function copyState(oldState) {
    return { tokenizer: oldState.tokenizer, lineState: oldState.lineState.copy(),
             ...
             expectingCaseVariant: oldState.expectingCaseVariant,
             expectingCaseParams: oldState.expectingCaseParams,
             blockStack: oldState.blockStack ? [...oldState.blockStack] : [],
  ...
  var external = {
    startState: function(basecolumn) {
      return {
        ...
        expectingCaseVariant: false,
        expectingCaseParams: false,
        blockStack: [],
        paramParenDepth: 0
      };
    },
