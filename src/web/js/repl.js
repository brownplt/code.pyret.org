  ...
  pyRepl, runtimeLib, replSupport, builtin, cpoBuiltin, gdriveLocators, compileStructs) {
    ...
    runtime.runThunk(function() {
      ...
      if(runtime.isSuccessResult(jsRepl)) {
        loadReplUI.then(function(accept) {
          accept(jsRepl.result);
        });
        loadReplUI.fail(function(err) {
          console.error("Error loading REPL UI: ", err);
        });
      }
      else {
