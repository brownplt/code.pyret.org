                              ...
                              return cases(
                                ...
                                RUNTIME.string_to_number(numSamplesC.val()),
                                {
                                  ...
                                  some: function (numSamplesVal) {
                                    ...
                                    if (
                                      !isTrue(
                                        RUNTIME.num_is_integer(numSamplesVal)
                                      ) ||
      ...
      return {
        ...
        overlay: (overlay, restarter, chart, container) => {
          ...
          function getBoundControl(control, name) {
            control.val(
              prettyNumToStringDigits5(get(get(globalOptions, name), "value"))
            );
            return $("<p/>")
      ...
      return RUNTIME.makeFunction((globalOptions, rawData) => {
        ...
        function setup(restarter) {
          ...
          tmp.options = $.extend(
            {},
            options,
            "options" in tmp ? tmp.options : {}
          );

          if ("overlay" in tmp)
            tmp.overlay(overlay, restarter, tmp.chart, root);
